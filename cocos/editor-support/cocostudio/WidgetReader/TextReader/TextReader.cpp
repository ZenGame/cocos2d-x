...
namespace cocostudio
{
    ...
    void TextReader::setPropsFromJsonDictionary(Widget *widget, const rapidjson::Value &options)
    {
		...
		if (FileUtils::getInstance()->isFileExist(fontFilePath))
		{
            ...
            std::string fontFullPath = FileUtils::getInstance()->fullPathForFilename(fontName);
            if (FileUtils::getInstance()->isFileExist(fontFullPath))
                label->setFontName(fontFullPath);
            else
